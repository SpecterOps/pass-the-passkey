...
Coming from the field of enterprise security, privilege escalation and lateral movement
by attacking Windows Integrated Authentication are our bread and butter.
But as more companies adopt cloud services, we decided to shift our attention to passkeys,
...
We identified a passkey implementation in a major cloud service that was vulnerable to the attacks the solution was designed to prevent. Moreover, we discovered that past signatures generated by YubiKeys were stored in cleartext and were readable by authenticated, unprivileged users, including remote users. This chain of vulnerabilities allowed us to successfully impersonate privileged users while bypassing the enforcement of phishing-resistant MFA and remaining undetected by popular XDR solutions.
...
The tooling we developed to exploit these vulnerabilities can also be used to perform passkey phishing, tampering, spoofing, fuzzing, and prompt flooding attacks. Some of these techniques can even be executed on compromised terminal hosts or virtual machines that target identities connect to. We will demonstrate the feasibility of these attacks using a popular C2 infrastructure.

Because the WebAuthn specification mandates a 22-step passkey validation process involving non-trivial cryptography and transactional processing, it is easy to make mistakes while implementing the spec, even for companies that co-authored the standard. We expect that by open-sourcing our tools, we will enable other penetration testers to discover many more web application vulnerabilities stemming from non-compliant passkey verification procedures.
...
by a Windows C2 agent like [Apollo](https://docs.specterops.io/mythic-agents/apollo-docs/documentation-payload/apollo) ([Mythic](https://mythic-c2.net/)).
...
`Passkey UI` is a simple Windows application that provides a graphical user interface for interacting with the Windows WebAuthn API.

![Passkey UI Screenshot](Assets/passkey-ui.png)

### DSInternals.Passkeys PowerShell Module

The `DSInternals.Passkeys` PowerShell module lets Microsoft Entra ID and Okta administrators register passkeys on behalf of other users.
...
The event includes the browser process ID and user SID, but the most important part is the JSON payload:
...
> The filter XML above accounts for this quirk. Unfortunately, it only works in PowerShell or the Win32 API, but not in the Event Viewer GUI.
...
The exploit would satisfy the phishing-resistant multifactor authentication requirement enforced by conditional access policies,
...
| 2026-04-15   | Microsoft confirmed the vulnerability and scheduled a fix for July 14. |
...
We tested multiple cloud services and identified Microsoft Entra ID as particularly vulnerable to replay attacks.
By itself, this vulnerability is not especially severe, as a malicious actor would first need to intercept HTTPS traffic
...
The Microsoft Entra ID engineering team decided to forgo certain security
...
The JWT payload indicates that the challenge is valid for only five minutes.
However, our tests have shown that Microsoft Entra ID accepts challenges
up to ten minutes after issuance.
Using an expired challenge would result in an authentication failure:

![Screenshot of an Entra ID challenge expired error](Assets/azure-challenge-expired.png)

However, the ten-minute validity period provides a sufficient window
...
In hybrid environments, this property is synchronized to the `msDS-KeyCredentialLink` user attribute in Active Directory Domain Services (AD DS) using Microsoft Entra Connect.
...
an adversary who acts fast enough might still be able to forward the WebAuthn assertion
before the legitimate user can. A network outage, browser freeze, OS hang,
...
Not currently possible because access is denied when attempting to overlay the Windows WebAuthn process.
...
The `hWnd` parameter is the handle of the window that will be used to display the UI.
We found that this parameter is not properly validated, allowing attackers to inject the passkey authentication prompt
...
The attack may result in unauthorized access to the user's cloud accounts, such as Microsoft Entra ID or GitHub.

#### Requirements

The adversary must be able to execute code on the target Windows 11 machine, e.g., through a malicious application delivered in a phishing campaign
...
2. Next, the attacker would need to correlate the retrieved passkeys with the running processes to find the window handle of the target application, e.g., a web browser.
...
3. Finally, the adversary can inject the passkey authentication prompt into the context of the target application
   by providing the corresponding window handle in the `hWnd` parameter of the `WebAuthNAuthenticatorGetAssertion` function:

```cmd
SharpPasskeys.exe prompt --relying-party github.com --challenge HvxwEkeqxPh-fB_c-wqXvfXiFXiamcEGluyRXSo2XxY --hwnd 2432736
```

![Credential UI Window Handle Injection](Assets/credential-ui-window-handle-injection.png)

Windows will display the passkey authentication prompt as a modal dialog of the target application,
making users more likely to confirm the request without noticing anything suspicious.
The malicious actor can then use the obtained WebAuthn assertion to impersonate the user and access their cloud accounts.

#### Conclusion

This attack seems especially practical in situations where the passkey is hardware-bound, as it can bypass phishing-resistant multifactor authentication requirements in conditional access policies.
...
When automation tools or scripts are used after successful passkey authentication,
OAuth 2.0 access tokens must be obtained first.

#### Attack Execution

The traditional way to fetch tokens from a browser session is opening the Developer Tools
...
The Passkey Injector tool can therefore invoke the built-in passkey authentication prompt as well.
...
- Properly implement all server-side checks according to the WebAuthn specification.
...
and allows attackers to register their own authenticators instead of legitimate ones.
...
This paper describes a side-channel vulnerability in the cryptographic
implementation of the Google Titan Security Key's secure element (CVE-2021-3011).
The authors show how to use a custom lattice-based attack to fully recover an ECDSA
private key from the Google Titan Security Key.
...
Use Evilginx to inject challenges into HTTPS traffic.
