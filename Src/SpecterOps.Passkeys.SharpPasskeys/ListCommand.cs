...
internal static class ListCommand
{
    ...
    {
        var allPrivateOption = new Option<bool>("--private", "-p")
        {
            Description = "Indicate that the browser is in private mode when enumerating Windows Hello credentials (OPSEC)"
        };
        ...
        {
            bool browserInPrivateMode = parseResult.GetValue(allPrivateOption);
            ListPlatformAuthenticators(logger);
            ListAuthenticatorPlugins(logger);
            ListWindowsHelloCredentials(logger, browserInPrivateMode);
        ...
        var helloPrivateOption = new Option<bool>("--private", "-p")
        {
        ...
        helloCommand.SetAction(parseResult => ListWindowsHelloCredentials(logger, parseResult.GetValue(helloPrivateOption)));
    ...
    /// </summary>
    private static void ListWindowsHelloCredentials(ILogger logger, bool browserInPrivateMode = false)
    {
        logger.LogInformation("Enumerating Windows Hello credentials...");

        try
        {
            var credentials = WebAuthnApi.GetPlatformCredentialList(null, browserInPrivateMode);
