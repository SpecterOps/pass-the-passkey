...
internal static class PromptCommand
{
    ...
    public static Command Create(ILogger logger)
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            if (flood)
            {
                RunFlood(logger, relyingParty, challengeBytes, allowedCredentials, credentialHints, attachment, windowHandle, browserInPrivateMode);
            }
            else
            {
                ...
                try
                {
                    string result = Prompt(rpId: relyingParty, challenge: challengeBytes, allowedCredentials, credentialHints, attachment, windowHandle, browserInPrivateMode);
    ...
    /// </summary>
    private static (string[]? Hints, AuthenticatorAttachment Attachment) ResolveCredentialHints(PublicKeyCredentialHint hint)
    {
        return hint switch
        {
            PublicKeyCredentialHint.SecurityKey => ([PublicKeyCredentialHint.SecurityKey.ToJsonString()!], AuthenticatorAttachment.CrossPlatform),
            PublicKeyCredentialHint.ClientDevice => ([PublicKeyCredentialHint.ClientDevice.ToJsonString()!], AuthenticatorAttachment.Platform),
            PublicKeyCredentialHint.Hybrid => ([PublicKeyCredentialHint.Hybrid.ToJsonString()!], AuthenticatorAttachment.CrossPlatform),
        ...
        bool browserInPrivateMode)
    {
        ...
        while (stopwatch.Elapsed < FloodTimeout)
        {
            ...
            try
            {
                string result = Prompt(rpId, challenge, allowedCredentials, credentialHints, attachment, windowHandle, browserInPrivateMode);
                Console.WriteLine(result);
                return;
            }
    ...
    private static WindowHandle ResolvePromptWindowHandle(ILogger logger, long? hwnd)
    {
        ...
        logger.LogInformation("Browser window not found; using the foreground window handle.");
