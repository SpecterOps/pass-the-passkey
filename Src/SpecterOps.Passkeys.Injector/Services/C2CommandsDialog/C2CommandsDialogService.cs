...
public sealed class C2CommandsDialogService : IC2CommandsDialogService
{
    ...
    /// <inheritdoc/>
    public void Show(PublicKeyCredentialRequestOptions assertionOptions, string currentAddress)
    {
        ArgumentNullException.ThrowIfNull(assertionOptions);
        ArgumentNullException.ThrowIfNull(currentAddress);

        IC2CommandsDialogViewModel viewModel = new C2CommandsDialogViewModel(assertionOptions, currentAddress);
