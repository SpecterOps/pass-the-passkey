...
public partial class C2CommandsDialogViewModel : ObservableObject, IC2CommandsDialogViewModel
{
    ...
    private IReadOnlyList<string> _credentialIds = [];

    /// <summary>
    /// Gets or sets the parsed assertion options.
    /// </summary>
    [ObservableProperty]
    ...
    public C2CommandsDialogViewModel() => SelectedAuthenticatorTypeHint = PublicKeyCredentialHint.None;
    ...
    partial void OnAssertionOptionsChanged(PublicKeyCredentialRequestOptions? value)
    {
        ...
        _rpId = ValidateRelyingPartyId(value.RpId);
        _challenge = ValidateBase64UrlToken(value.Challenge, nameof(value.Challenge));

        List<string> credentialIds = [];
        if (value.AllowCredentials is { Length: > 0 } allowCredentials)
        {
        ...
        SelectedAuthenticatorTypeHint = ResolveDefaultHint(value.Hints);
        RebuildAllCommands();
    }
